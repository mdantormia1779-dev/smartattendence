...
export async function GET(request: Request) {
  try {
    ...
    const totalClicks = accounts.reduce((sum: number, a: any) => sum + (a.totalClicks || 0), 0);
    const totalRegistrations = accounts.reduce((sum: number, a: any) => sum + (a.totalRegistrations || 0), 0);
    const totalPaidCustomers = accounts.reduce((sum: number, a: any) => sum + (a.totalPaidCustomers || 0), 0);
    const totalReferralRevenue = accounts.reduce((sum: number, a: any) => sum + (a.totalRevenue || 0), 0);
    const pendingCommissionsTotal = accounts.reduce((sum: number, a: any) => sum + (a.pendingCommission || 0), 0);
    const availableCommissionsTotal = accounts.reduce((sum: number, a: any) => sum + (a.availableBalance || 0), 0);
    const totalPayoutsDistributed = accounts.reduce((sum: number, a: any) => sum + (a.paidCommission || 0), 0);
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "PROCESS_PAYOUT") {
      ...
      if (!result.success) {
        return apiError(new Error(result.error || "Failed to process withdrawal payout"));
      }
