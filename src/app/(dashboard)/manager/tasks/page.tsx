...
export default function ManagerTasksPage() {
  ...
  const fetchData = async (showRefresh = false) => {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        if (rawUser) {
          try {
            ...
            activeUserId = parsed.id || parsed.userId || "";
          } catch {}
        }
        if (!activeOrgId) {
      ...
      // Handle employees correctly (support { items: [...] }, { data: { items: [...] } }, or array)
      ...
      let mapped: TeamMemberOption[] = rawEmployees.map((e: any, idx: number) => ({
        ...
        department: e.department || e.departmentName || e.departments?.name || "Operations",
        branch: e.branch || e.branchName || e.branches?.name || "Main Branch",
        avatar: e.image || e.profilePicture || e.avatar || null,
      }));

      // Fallback sample members if none yet
      if (mapped.length === 0) {
        mapped = [
      ...
      console.error("[ManagerFetchTasksError]", err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setFormError(res.message || res.error?.message || "Failed to assign task");
      }
    } catch (err: any) {
  ...
  const handleStatusChange = async (task: TaskItem, newStatus: TaskItem["status"]) => {
    try {
      const res = await api.tasks.update(task.id, {
        status: newStatus,
        completionNotes: newStatus === "COMPLETED" ? (task.completionNotes || "Approved by Manager") : task.completionNotes,
      });
      ...
      console.error("[ManagerUpdateTaskStatusError]", err);
    }
  };
    ...
    <div ref={containerRef} className="flex-1 bg-[#FBFBFA] p-4 sm:p-6 md:p-8 space-y-6 overflow-y-auto min-h-screen text-stone-800">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-stone-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-1">
            Delegate responsibilities to your team members, track deliverable status, and review completion feedback.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        <div className="anim-card p-5 rounded-2xl bg-white border border-stone-200/80 shadow-xs">
          <div className="flex items-center justify-between text-stone-500 mb-2">
            <span className="text-xs font-semibold uppercase tracking-wider">Team Tasks</span>
            <Layers className="w-4 h-4 text-[#00B050]" />
          </div>
          <div className="text-2xl sm:text-3xl font-bold text-stone-900">{stats.total}</div>
          <div className="text-[11px] text-stone-400 mt-1">Total assigned in team</div>
        </div>
        ...
        <div className="anim-card p-5 rounded-2xl bg-white border border-stone-200/80 shadow-xs">
          ...
          <div className="text-[11px] text-stone-400 mt-1">Not started yet</div>
        </div>
        ...
        <div className="anim-card p-5 rounded-2xl bg-white border border-stone-200/80 shadow-xs">
          ...
          <div className="text-[11px] text-stone-400 mt-1">Currently being worked on</div>
        </div>
        ...
        <div className="anim-card p-5 rounded-2xl bg-white border border-stone-200/80 shadow-xs">
          ...
          <div className="text-[11px] text-stone-400 mt-1">Verified and done</div>
        </div>
        ...
        <div className="anim-card p-5 rounded-2xl bg-rose-50/50 border border-rose-200/80 shadow-xs col-span-2 sm:col-span-1">
          ...
          <div className="text-[11px] text-rose-500 mt-1">Needs immediate follow up</div>
        </div>
      </div>
      ...
      <div className="anim-card p-4 rounded-2xl bg-white border border-stone-200/80 shadow-xs flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search team task title, description, or member..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-stone-50/70 border border-stone-200 rounded-xl pl-10 pr-4 py-2 text-xs sm:text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-[#00B050] focus:ring-1 focus:ring-[#00B050]"
          />
          {searchQuery && (
            <button
      ...
      {loading ? (
        <div className="py-20 flex flex-col items-center justify-center text-stone-400 gap-3">
          ...
          <p className="text-xs font-semibold">Loading team task registry...</p>
        </div>
        ...
        <div className="py-16 text-center rounded-3xl bg-white border border-stone-200/80 shadow-xs flex flex-col items-center justify-center gap-3">
          ...
          <h3 className="text-base font-semibold text-stone-800">No team tasks found</h3>
          <p className="text-xs text-stone-500 max-w-sm">
            {searchQuery || selectedStatus !== "ALL" || selectedPriority !== "ALL"
              ? "No tasks match your filter. Try adjusting your search query."
              : "No tasks assigned to your team members yet. Click 'Assign Team Task' to delegate work."}
          </p>
        </div>
      ) : (
          ...
          <div className="w-full max-w-xl bg-white border border-stone-200 rounded-3xl shadow-2xl p-6 sm:p-7 space-y-6 relative">
            <div className="flex items-center justify-between pb-4 border-b border-stone-100">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-lg font-bold text-stone-900">Assign Team Task</h2>
                  <p className="text-xs text-stone-500">Delegate deliverable to a member of your team</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleCreateTask} className="space-y-4">
              <div>
                ...
                >
                  <option value="" disabled>Choose member ({teamMembers.length} available)...</option>
                  {teamMembers.map((m) => (
                    <option key={m.id} value={m.id}>
                      {m.fullName} ({m.employeeCode}) — {m.designation} [{m.department}]
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Prepare monthly dispatch report"
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1.5">
                  Deliverable Instructions / Criteria
                </label>
                <textarea
                  rows={3}
                  placeholder="Specify key requirements, guidelines, and expected deliverables..."
          ...
          <div className="w-full max-w-xl bg-white border border-stone-200 rounded-3xl shadow-2xl p-6 sm:p-7 space-y-6 relative">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="p-3 rounded-2xl bg-stone-50 border border-stone-200/80">
                  <span className="text-stone-400 block text-[10px] uppercase font-bold">Current Status</span>
                  <span className="text-stone-900 font-bold">{selectedTask.status}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Add feedback or notes on this task deliverable..."
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-stone-100">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold bg-[#00B050] hover:bg-[#009b46] text-white shadow-md shadow-[#00B050]/20 transition-all cursor-pointer disabled:opacity-50"
              >
                Save Feedback
              </button>
            </div>
